...
public class T5Semantico extends t5GramBaseVisitor<Void> {

    // Tabela que armazena os escopos gerados ao longo da análise
    TabelaDeSimbolos tabela;

    // Geração de um conjunto de escopos que serão analisados de forma auxiliar
    static Escopos escoposAninhados = new Escopos();

    // Tabela auxiliar que armazenará os nomes e parâmetros das
    // funções e procedimentos
    static HashMap<String, ArrayList<TipoT5>> dadosFuncaoProcedimento = new HashMap<>();

    // Tabela que armazenará as variaveis pertencentes a um registro
    ...
    public Void visitPrograma(t5GramParser.ProgramaContext ctx) {
        ...
        // Verifica se há alguma utilização do comando retorne no escopo do programa
    ...
    @Override
    public Void visitCorpo(t5GramParser.CorpoContext ctx) {
        ...
        if (ctx.getText().contains("declare")) {
            ...
            if (ctx.variavel().tipo().registro() != null) {
                ...
                if (tabelaRegistro.containsKey(tipoVariavel)) {
                    ...
                    for (t5GramParser.IdentificadorContext ident : ctx.variavel().identificador()) {
            ...
            if (ctx.tipo().registro() != null) {
                ...
                for (t5GramParser.VariavelContext vc : ctx.tipo().registro().variavel()) {
                    tipoVariavel = vc.tipo().getText();

                    for (t5GramParser.IdentificadorContext ic : vc.identificador()) {
    ...
    public Void visitDeclaracao_global(t5GramParser.Declaracao_globalContext ctx) {
        ...
        if (ctx.getText().contains("procedimento")) {
            ...
            for (t5GramParser.ParametroContext parametro : ctx.parametros().parametro()) {
                ...
                if (parametro.tipo_estendido().tipo_basico_ident().tipo_basico() != null) {
                    ...
                    // Obtém os tipos dos parâmetros para adicioná-las na tabela de dados de funções
                    // e procedimentos
                    ...
                    // Verifica se é um registro
                        ...
                        .containsKey(parametro.tipo_estendido().tipo_basico_ident().IDENT().getText())) {
                    // Recupera os elementos do registro
                    variaveisRegistro = tabelaRegistro
                            .get(parametro.tipo_estendido().tipo_basico_ident().IDENT().getText());
            ...
            // A partir daqui, utiliza uma lógica semelhante às verificações para o
            // procedimento
            for (t5GramParser.ParametroContext parametro : ctx.parametros().parametro()) {
    ...
    public Void visitCmdEscreva(t5GramParser.CmdEscrevaContext ctx) {
        ...
        for (t5GramParser.ExpressaoContext expressao : ctx.expressao())
    ...
    public Void visitCmdAtribuicao(t5GramParser.CmdAtribuicaoContext ctx) {
        ...
        if (tipoExpressao != TipoT5.INVALIDO) {
            ...
            if (!tabela.existe(varNome))
                ...
                // Caso o tipo seja inteiro ou real, é utilizada a função
                ...
                if (varTipo == TipoT5.INTEIRO || varTipo == TipoT5.REAL) {
                    // Verifica se a variável atual é um ponteiro para apresentar uma mensagem
                    // personalizada
                    // para este caso
                    if (ctx.getText().contains("ponteiro")) {
                        if (!verificaCompatibilidade(varTipo, tipoExpressao))
                            // Caso o tipo da expressão (restante da parcela sendo analisada) seja diferente
                            // de inteiro,
                            // não é possível tratar o valor como um número real, logo, os tipos são
                            // incompatíveis, pois
